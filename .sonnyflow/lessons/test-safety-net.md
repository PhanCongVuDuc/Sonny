...
| `BeamSectionListParserTests` / `FramingSymbolSizingPolicyTests` ([source](../../source/Sonny.Application.UnitTests/Domain/FramingFromCad/)) | The parse rule and the naming rule of [FramingFromCad.md](../../docs/features/FramingFromCad.md) — including the **silent skip** of every malformed section entry, and the type-name shape `200x300` (no spaces, no unit, lowercase `x`) which differs from the column feature's `200 x 300mm` | A test here is changed. Every case maps to a Contract rule; the silent skips are the requirement. Note one case deliberately probes the tolerance boundary as `0` vs `0.001` — `1.0` vs `1.001` asserts nothing, because that subtraction lands below the tolerance in `double` |
| `FramingFromCadInteractorTests` ([source](../../source/Sonny.Application.UnitTests/UseCases/FramingFromCad/FramingFromCadInteractorTests.cs)) | The three Domain ports (`IFramingDataExtractor`, `IBeamCreator`, `IBeamJustificationAdjuster`), the resource-key passthrough convention, and **two transactions distinguished by creation order** (first = beam pass, second = justification) — an assertion about one cannot be satisfied by the other | A port signature changes (wiring-only retarget allowed). Two tests pin **kept bugs** ([FFC-001](../../docs/bugs/FFC-001-single-line-pass-crashes-when-no-symbol-resolved.md), [FFC-002](../../docs/bugs/FFC-002-duplicate-type-name-loses-a-whole-section-silently.md)) — making them behave nicely requires changing the Contract first |
| `FramingFromCadIntegrationTest` ([source](../../source/Sonny.Application.Tests/Features/FramingFromCad/IntegrationTests/FramingFromCadIntegrationTest.cs)) | Constructs the UseCases `FramingFromCadInteractor` **by hand** (`FakeProgressReporter`, `FakeMessageService`, `ImmediateRevitTaskRunner` from `TestDoubles.cs`); finds the CAD link, family, types and level **by name or by being the only one of their kind** — no `UniqueId` is pinned; expected counts come from `FramingFromCadFixtureFacts`, every one of them **measured** by running the feature over the supplied DWG. Must stay a **synchronous** test: NUnit runs an async test under its own synchronization context and the continuation after the first `await` leaves Revit's API context, so the next transaction fails with "changes to the document are temporarily disabled". Two counts look wrong and are not — 30 pairs at width 300 (12 of them end-cap pairs) and 0 pairs at width 350 with single-line mode on (the 300 section ate them) | The interactor constructor changes; the DWG is replaced without re-measuring `FramingFromCadFixtureFacts`; someone makes the test `async`, or "corrects" the 30/0 expectations |
| `FramingFromCadFixtureBuilder` ([source](../../source/Sonny.Application.Tests/Features/FramingFromCad/IntegrationTests/FramingFromCadFixtureBuilder.cs)) | The placeholder project as seed — it already carries `M_Concrete-Rectangular Beam` with `b`/`h` type parameters, so **no family authoring is needed**; the drawing is the project owner's own Revit-exported `Resources/RevitFiles/Dwgs/Test_V2023_FramingFromCad.dwg`, imported with the 4-argument `Document.Import` overload in millimetres | Runs only when `Test_V2023_FramingFromCad.rvt` is missing and the process is Revit 2023 — otherwise `Assert.Ignore`. Delete the fixture to rebuild it; the builder self-verifies four layer stroke counts and all four pair counts with the feature's own helpers before saving. **Do not replace the DWG without re-measuring** — an earlier self-generated DXF was dropped precisely because it was too clean to exercise the quirks |
...
|---|---|---|
| `SettingsService` — language and display-unit persistence, the cache, and the change events | Its only fixture (`SettingsServiceLanguageTests`) was **deleted** by [ADR 0002](../../docs/adr/0002-revit-free-tests-live-in-unittests.md). It drove the real `%APPDATA%\Sonny\SonnySettings.json`, so running the suite destroyed whatever language the developer had chosen, and the nine tests shared that one file in an order-dependent way | `SettingsService` hard-codes its path in the constructor. Give it an injectable directory (or an `ISettingsFileLocation` port) and the fixture becomes an ordinary temp-folder unit test in `Sonny.Application.UnitTests` |
...
- [ADR 0001](../../docs/adr/0001-decision-logic-lives-in-usecases.md) — the refactor where the
  `AutoColumnDimensionIntegrationTest` wiring retarget happened, and the bit-compatibility constraint was
  discovered
- [ADR 0002](../../docs/adr/0002-revit-free-tests-live-in-unittests.md) — why the Revit-free fixtures moved out of
